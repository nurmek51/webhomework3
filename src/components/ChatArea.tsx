...
function ChatArea({ selectedChat, updateChatMessages, updateChatLastMessagePreview, onBackToList }: ChatAreaProps) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-bg-secondary text-text-primary">
      {/* Chat Header */}
      <div className="sticky top-0 z-10 bg-primary-color border-b border-border-custom px-4 py-3 shadow-sm text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            {/* Back button for mobile */}
            <button 
              className="md:hidden p-2 text-white hover:bg-dark-teal rounded-full transition-colors"
            ...
            <div className="relative">
              ...
              {selectedChat.type === 'person' && (
                <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${
                  selectedChat.isOnline ? 'bg-green-500' : 'bg-status-offline'
                }`} />
              )}
            </div>
            ...
            <div>
              <h2 className="font-semibold text-white flex items-center">
                {selectedChat.name}
                {selectedChat.type === 'ai' && (
                  <Bot size={14} className="ml-1 text-light-teal" />
                )}
              </h2>
          ...
          <div className="flex items-center space-x-2">
            {selectedChat.type === 'person' && (
              <>
                <button className="p-2 text-white hover:bg-dark-teal rounded-full transition-colors">
                  <Phone size={18} />
                </button>
                <button className="p-2 text-white hover:bg-dark-teal rounded-full transition-colors">
                  <VideoIcon size={18} />
      ...
      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-bg-secondary">
      ...
      {/* Message Input Area */}
      <div className="bg-bg-primary border-t border-border-custom px-4 py-3">
        <div className="flex items-end space-x-2">
          <button className="p-2 text-text-secondary hover:bg-hover-bg rounded-full transition-colors">
            <Paperclip size={20} />
          </button>
          
          <textarea
            ref={inputRef}
            className="flex-1 resize-none text-text-primary text-sm py-2 px-3 rounded-lg border border-border-custom focus:ring-1 focus:ring-primary-teal bg-bg-secondary"
          ...
          <button 
            className="p-2 bg-primary-color text-white rounded-full disabled:opacity-50 disabled:cursor-not-allowed hover:bg-dark-teal transition-colors"
